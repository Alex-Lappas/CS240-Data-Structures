...
extern struct SubInfo *S; /*this might be a mistake to do depending on compile style but just to be sure and to make compiler stop complaining about this file*/
...
int Consume(int sId)
{
	...
	while (temp)
	{
		int reps_in_table =0;
		if(temp->sId==sId){ 
			for(reps_in_table;reps_in_table<MG;++reps_in_table){ 
				if(temp->sgp[reps_in_table]!=(struct Info *)1){ 
					DL_Print_consume(G[reps_in_table].gfirst);
					temp->sgp[reps_in_table]= G[reps_in_table].glast;
				}
			}
		}
		temp=temp->snext;
...
int Print_all(void)
{
	...
	while (temp)
	{
		...
		printf("Subscriber %d is interested in information from Group with id =<", temp->sId);
